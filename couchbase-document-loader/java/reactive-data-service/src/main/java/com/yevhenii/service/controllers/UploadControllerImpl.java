...
import com.yevhenii.service.services.StreamService;
...
public class UploadControllerImpl implements UploadController {
    ...
    @Override
    @RequestMapping(method = RequestMethod.GET, path = "/data/upload/stream")
    public ResponseEntity<Integer> streamUpload() {
        try {
            return ResponseEntity.ok(
                    streamService.upload().size()
            );
        } catch (IOException e) {
            return ControllerUtils.failWithLogging(e,
                    "Error occurred during processing sequential upload", log);
